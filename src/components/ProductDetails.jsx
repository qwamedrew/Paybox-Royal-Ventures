...
const ProductDetails = ({ product, onInquire, onBack }) => {
  const {
    id,
    name,
    images = [],
  ...
  const [selectedImage, setSelectedImage] = useState(mainImage || (images.length > 0 ? images[0] : ''));
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                {price ? (
                  <div className="text-2xl font-bold text-green-700">
                    ${price} <span className="text-base font-normal text-gray-600">/ {unit}</span>
                  </div>
                ) : (
                  <div className="text-lg italic text-gray-600">Price upon request</div>
                )}
                {minOrder && (
                  <div className="text-sm text-gray-600 mt-1">Minimum order: {minOrder} {unit}</div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {seasonality && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <h3 className="text-sm font-semibold text-gray-700 mb-1">Seasonality</h3>
